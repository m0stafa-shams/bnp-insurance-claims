## ----------------------------------------------------------------------------
## This is a R code for the PYMM model applied to claims frequency
...
neal_algorithm8 <- function(beta0, m, alpha0, d0, n_iter) {
  ...
  for (k in 2:n_iter) {
    for (i in 1:n) {
      ...
      for (l in 1:k_minus) {
        prob[l] <- (log(n_i(c, l, i) - d) - exp(X[i, ] %*% phi[l, ]) + ncl[i] *
          (X[i, ] %*% phi[l, ]))
      }

      for (l in (k_minus + 1):h) {
        prob[l] <- (log((alpha + d * k_minus) / m) - exp(X[i, ] %*% phi[l, ]) + ncl[i] * (X[i, ] %*% phi[l, ]))
      }
...
pred <- function(nclnew, xnew) {
  ...
  for (t in (burnin + 1):n_iter) {
    ...
    part2_a <- 0
    for (v1 in 1:n) {
      part2_a <- (part2_a + exp(f(temp_theta[v1, ])))
    }

    temp_theta_star <- unique(beta1[, ((vn) * t - (vn) + 1):((vn) * t)])
    part2_b <- 0
    for (v2 in 1:ndistinct1[t]) {
      part2_b <- (part2_b + exp(f(temp_theta_star[v2, ])))
    }
...
pr <- foreach(nclnew = 0:my, .combine = "c") %do% {pred(nclnew = nclnew, xnew = xnew)}
...
observed = as.numeric(dyt[,2])

# MSE and SE
MSE = mean((observed - expected)^2)
SE_MSE = sd((observed - expected)^2)/sqrt(length((observed - expected)))
...
legend("topright", 
       legend = c("Predictive", "Observed"),
       fill = c("steelblue4", "tomato"),
       border = "gray30",
       cex = 1.2,         # larger text
       x.intersp = 1.4,   # add horizontal spacing
       y.intersp = 1.3,   # add vertical spacing
       box.lwd = 1.5,     # thicker border
       box.col = "gray40" # border color
)
